...
def verify_password(stored: str, supplied: str) -> bool:
    try:
        return ph.verify(stored, supplied)
...
def current_user(s: LoginSession = Depends(current_session)) -> User:
    return s.user
...
def teacher(user: User = Depends(current_user)) -> User:
    if user.role == "admin":
        return user
    if user.role != "teacher" or not user.teacher_approved:
...
def csrf(request: Request, s: LoginSession = Depends(current_session)) -> LoginSession:
    ...
    if not secrets.compare_digest(given, s.csrf_token):
